...
def validate_and_save(container: dict, status: str, notes: str, sales_stage: str | None = None, advance: bool = False, message_container=None) -> bool:
    """
...
def render():
            ...
            if not filtered:
                ...
                with save_col:
                    if st.button("Save", type="secondary"):
                        if selected_status:
                            if validate_and_save(selected, selected_status, notes, selected_sales_stage or None, advance=False, message_container=message_container):
                                st.rerun()
                        else:
                            with message_container:
                ...
                with save_next_col:
                    ...
                    if st.button(next_label, type="primary"):
                        if selected_status:
                            if validate_and_save(selected, selected_status, notes, selected_sales_stage or None, advance=not is_last_item, message_container=message_container):
                                st.rerun()
                        else:
                            with message_container:
                                st.error("Please select a status.")
    ...
    render_page_frame(
        title="Scrubbing",
        subtitle="Queue-based triage workflow",
        content_callable=_render_content,
        footer_text="Stage 2 | Training Catalogue Manager"
    )
